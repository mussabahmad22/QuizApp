...
class AdminController extends Controller
{
    ...
    public function categoury_delete(Request $request)
    {
        ...
        $result = Exercise::where('categoury_id', $categoury_id)->get();
        ...
        if (count($result) > 0) {
            ...
            $del_ans_exe_id = Answer::where('exercise_id', $del_exe->id);
            $del_ans_exe_id->delete();
            $del_ans_ques_id = Answer::where('question_id', $del_exe->id);
            $del_ans_ques_id->delete();
            $del_ques = Question::where('exercise_id', $del_exe->id);
            $del_ques->delete();
            $del_quizez_exe_id = Quiz::where('exercise_id', $del_exe->id);
            $del_quizez_exe_id->delete();
